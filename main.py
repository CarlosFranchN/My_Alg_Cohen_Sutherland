...
def main(page: ft.Page):
    ...
    def desenhar_vetor(ponto1, ponto2):
        ...
        vetor.clip()
        new_vetor = vetor.vetor_viewport

        escala = VIEWPORT_SIZE / 50

        ponto1_canvas = (
            new_vetor[0][0] * escala,
            VIEWPORT_SIZE - new_vetor[0][1] * escala,
        )
        ponto2_canvas = (
            new_vetor[1][0] * escala,
            VIEWPORT_SIZE - new_vetor[1][1] * escala,
        )
    ...
    def calcular_vetor(e):
        try:
            ...
            ponto1 = (float(ponto1_x.value), float(ponto1_y.value))
            ponto2 = (float(ponto2_x.value), float(ponto2_y.value))

            vetor = Vetor(ponto1, ponto2)
            vetor.clip()
            result_label.value = f"Vetor: {vetor.vetor}\nBits do vetor: {vetor.bits_vetor}\n{vetor.contido()}\n Vetor renderizado: {vetor.vetor_viewport}"
